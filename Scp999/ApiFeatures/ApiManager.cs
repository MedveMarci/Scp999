...
public static class ApiManager
{
    ...
    {
        var name = Scp999.Singleton.Name;
        var currentVersion = Scp999.Singleton.Version;
    ...
    internal static string SendLogsAsync(string content)
    {
        try
        {
            var url = $"{ApiBase}/api/v1/plugin/{Uri.EscapeDataString(Scp999.Singleton.Name)}/log";
            LogManager.Info("Sending logs to BearmanAPI...", ConsoleColor.Green);

            var payload = new
            {
                content,
                plugin_version = Scp999.Singleton.Version.ToString(),
    ...
    internal static void SendAutoError(string errorMessage)
    {
        Task.Run(() =>
        {
            try
            {
                if (Scp999.Singleton?.Config == null) return;
